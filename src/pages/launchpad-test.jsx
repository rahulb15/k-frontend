...
[
  ...
  const saleFunction = async () =>
    ...
    // saleType
    {
      ...
      if (localResponseKm.result.status == "success" && localResponseLedger.result.status == "success") {
        let signedTx1;
        let signedTx2;
        console.log(wallet);
        if (wallet == "ecko") {
          [signedTx1,signedTx2] = await eckoWallet([km_marketplace_txn,make_trx_fixed]);
          // signedTx2 = await eckoWallet(make_trx_fixed);
        }
        if (wallet == "CW") {
          [signedTx1,signedTx2] = await signWithChainweaver([km_marketplace_txn,make_trx_fixed]);
  ...
  return (
    <>
      ...
      <div>
        <button onClick={() => setSyncFormVisible(true)}>Sync With Ng</button>
        {isSyncFormVisible && (
