...
import { useTokenPolicies, useSalesForToken, useTokenBalance, useTokenSupply, usePrecision, useRoyalty } from "src/hooks/SWR_Hooks";
...
const NftMarketPlaceDetailModal = ({ open, onClose, data }) => {
  ...
  useEffect(() => {
    setWallet(accountuser?.user?.walletName === "Ecko Wallet" ? "Ecko" : accountuser?.user?.walletName === "Chainweaver" ? "ChainWeaver_Desktop" : "Other");
